...
void PeanutGL::EngineSetup( Engine* const engine ) {
    ...
    if ( vert_shader and frag_shader and SSBO and container ) {
        ...
        auto* const model_component{ text_entity->AddComponent< ModelComponent >( "model" ) };
        model_component->UseUniform();

        auto* const view_component{ text_entity->AddComponent< ViewComponent >( "view" ) };
        view_component->UseUniform();

        auto* const projection_component{ text_entity->AddComponent< ProjectionComponent >(
